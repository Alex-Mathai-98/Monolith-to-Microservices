...
class Edge2CommonSpace(nn.Module) :

    def __init__(self, common_space:int, edge_type_info : List[Tuple[str,int]]) :
        
        super(Edge2CommonSpace,self).__init__()
        ...
        for edge_type,edge_dim in edge_type_info :
            self.edge_matrices[edge_type] = nn.Parameter( torch.Tensor(self.common_space,edge_dim) )
            self.edge_matrices[edge_type + "_bias"] = nn.Parameter( torch.Tensor(self.common_space,1).squeeze() )
    ...
    def forward(self, edge_features : List[torch.tensor], edge_types : List[str]) :
        if len(edge_features)==0 and len(edge_types)==0 :
            return None
        num_edges = len(edge_types)
        ans = torch.zeros((num_edges,self.common_space))
